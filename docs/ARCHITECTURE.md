...
## Browser references and video context

`SelectionService` resolves `AXURL`/`AXDocument` along the focused element’s ancestors in known browsers, after rejecting secure controls. Manual invocation can create a page-only snapshot; automatic mouse-release invocation still requires a selection. `RequestCoordinator` collects the web reference and screenshot independently behind one readiness gate. Each has a five-second deadline and session/generation identity; neither can restart completed generation. Reference fetches are cancelled on reset, disablement, pause, or offline mode.

`PublicPageDownload` uses macOS curl with a validated/pinned public IPv4 destination, original-host TLS validation, disabled curl configuration/proxies/cookies/redirects, a four-second transport deadline and a 2 MB response cap. Private IPs are rejected before connecting. `WebReferenceService` parses fetched UTF-8 HTML in a nonpersistent WKWebView with page scripts, subresources, frames, and subsequent navigations blocked. Bundled Readability 0.6.0 extracts plain text. References carry URL, title, type and optional timestamp, with a separate 12,000-byte text budget. They are untrusted source material for prose and excluded from deterministic chart evidence.

The Manifest V3 extension requests activeTab, scripting and nativeMessaging; no standing host permissions. It reads a cloned loaded article or available HTML video cues/YouTube open-transcript rows. Captions retain timestamps, scope and truncation labels. It sends no AI requests. The Python native messaging host relays length-prefixed JSON to an authenticated loopback listener. The app validates payload size, age, duplicate ID, URL, browser identity and context policy. Connection metadata alone persists; source content does not. Native-host installation is user initiated and tied to the extension ID. Safari/Firefox packaging and live Chromium-variant compatibility remain pending.

`AudioContextService` is an explicit 30-second ScreenCaptureKit source-application audio stream, with an on-device SFSpeechRecognizer requirement, permission checks, cancellation, and no disk recording. No microphone input is attached. Other audio in the same browser app may be included. The transcript is a new local reference; it never triggers generation itself. Listening ends on cancellation, source-session replacement, settings-policy changes or the recording deadline. Live permission/audio behavior requires device validation.
